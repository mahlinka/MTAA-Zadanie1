...
HOST, PORT = '192.168.0.250', 5060  #192.168.0.250
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processInvite(self):
        logging.debug("-----------------")
        logging.debug(" INVITE prijaty ")
        logging.debug("-----------------")
        origin = self.getOrigin()
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Zly dopyt")

            return
        destination = self.getDestination()
        if len(destination) > 0:
            logging.info("destination %s" % destination)
            logging.info("Adresa %s vytocila adresu %s" %(origin, self.getDestination()))
    ...
    def processAck(self):
        ...
        destination = self.getDestination()
        if len(destination) > 0:
            logging.info("%s vola s %s" % (destination, self.getOrigin()))
    ...
    def processNonInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Zly dopyt")
            logging.warning("Neregistrovana adresa")
            return
        destination = self.getDestination()
        if len(destination) > 0:
            logging.info("destination %s" % destination)
            logging.info("Hovor bol ukonceny z adresy %s" %(origin))
            if destination in registrar and self.checkValidity(destination):
                socket,claddr = self.getSocketInfo(destination)
                #self.changeRequestUri()
                self.data = self.addTopVia()
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                code = data[0].split(" ")[1]
                if code == "603":
                    logging.info("Adresa %s neprijala hovor od adresy %s" %(self.getDestination(), origin))

                if (data[0]=="SIP/2.0 200 Ok" and data[5]=="CSeq: 20 INVITE"):
                    logging.info("Adresa %s prijala hovor od adresy %s" %(self.getDestination(), self.getOrigin()))
